...
/* Beschreibungen für Tooltip im Konferenz-Dropdown */
const confDescriptions = {
  "*ECHOTEST*": "Testserver: Sprache wird zurückgespielt",
  "*AMSAT*": "AMSAT: Amateurfunksatelliten / ISS",
  "*DODROPIN*": "DoDropIn: große offene Konferenz",
...
export default function App() {
  ...
  // ===== QRZ Location =====
  const [qrzLocation, setQrzLocation] = useState(null);
  ...
  useEffect(() => {
    ...
    const fetchAddresses = async () => {
      try {
        ...
        if (alive) {
          setNetworkAddresses(data.items || []);
        }
      } catch (error) {
        console.error("Failed to fetch network addresses:", error);
      }
      if (alive) setTimeout(fetchAddresses, 10000);
    };
    fetchAddresses();
    return () => { alive = false; };
  }, []);
  ...
  // ===== Laden: QRZ Location =====
  useEffect(() => {
    const fetchQrzLocation = async () => {
      if (!selectedLogin || selectedLogin.callsign.startsWith("*")) {
        setQrzLocation(null);
        return;
      }
      try {
        const response = await fetch(`/api/qrz/lookup?callsign=${selectedLogin.callsign}`);
        const data = await response.json();
        if (data.ok) {
          setQrzLocation(data.location);
        } else {
          setQrzLocation(null);
          console.error("QRZ lookup failed:", data.error);
        }
      } catch (e) {
        console.error("Failed to fetch QRZ data:", e);
        setQrzLocation(null);
      }
    };
    fetchQrzLocation();
  ...
  const connectConference = async () => {
    ...
    if (selectedConf.startsWith("*")) {
      ...
      if (match?.node) {
        ...
        alert("Für diese Konferenz wurde keine Node-ID gefunden. Bitte Logins aktualisieren und erneut versuchen.");
        return;
      }
    }
    if (!/^\d{3,}$/.test(serverToDial)) {
      alert("Ungültiger Zielwert. Bitte eine gültige Node-ID wählen.");
      return;
    }
    ...
    <div className="container">
      ...
      {status && typeof status.connectedTo !== "undefined" && status.connectedTo !== null && String(status.connectedTo).trim() !== "" && (
        <div className="banner">
          <strong>🔗 Verbunden mit:</strong> {String(status.connectedTo)}
        </div>
      )}
      ...
      <section className="card">
        <h2>Dienst</h2>
        <div className="grid2">
          <div className="info"><span>Status</span><strong>{status?.service ?? "…"}</strong></div>
          <div className="info"><span>SVXLink</span><strong>{status?.version ?? "—"}</strong></div>
          <div className="info"><span>Rufzeichen</span><strong>{status?.nodeCall ?? "—"}</strong></div>
          <div className="info">
            <span>Module</span>
            <strong>{(status?.modules?.map?.(m=>m.name)?.join(", ")) || "—"}</strong>
          </div>
          <div className="info"><span>Backend</span><strong>{status?.svxwebVersion ?? "—"}</strong></div>
      ...
      {/* Netzwerkadressen */}
      <section className="card">
        <h2>Netzwerkadressen</h2>
        {networkAddresses.length === 0 && <p className="muted">Lade Netzwerkadressen...</p>}
        <div className="grid2">
          {networkAddresses.map((addr, index) => (
            <div className="info" key={index}>
              <span>{addr.iface} ({addr.family})</span>
      ...
      <section className="card">
        ...
        <div className="row">
          <input
            ...
            placeholder="Schnellsuche (z. B. echotest, amsat …)"
            value={confQuery}
      ...
      <section className="card">
        ...
        <div className="row">
          ...
          <button className="btn ghost" disabled={loginsLoading} onClick={refreshLogins}>
            {loginsLoading ? "Lade…" : "Aktualisieren"}
          </button>
        </div>
        ...
        {qrzLocation && (
          <div className="grid2 mt-8">
            <div className="info"><span>QRZ-Ort</span><strong>{qrzLocation.city}, {qrzLocation.state}</strong></div>
            <div className="info"><span>QRZ-Land</span><strong>{(qrzLocation.country && qrzLocation.country.trim()) ? qrzLocation.country : 'unbekannt'}</strong></div>
            <div className="info"><span>Koordinaten</span><strong>{qrzLocation.latitude}, {qrzLocation.longitude}</strong></div>
          </div>
        )}
      </section>
