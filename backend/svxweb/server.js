...
// RoutenÃ¼bersicht beim Start ins Log
setTimeout(() => {
  try {
...
// ===== SVXLINK Log Endpunkte =====

// GET /api/logs/tail -> letzte N Zeilen aus /var/log/svxlink
app.get("/api/logs/tail", (req, res) => {
  const lines = parseInt(req.query.lines || "100", 10);
  try {
    const tail = spawn("tail", ["-n", String(lines), SVXLINK_LOG]);
    ...
    res.status(500).json({
      error: "Fehler beim Lesen des svxlink-Logs",
      details: err.message,
    });
  }
});

// GET /api/logs/stream -> Echtzeit-Streaming per SSE
app.get("/api/logs/stream", (req, res) => {
  ...
  const tail = spawn("tail", ["-n", "0", "-f", SVXLINK_LOG]);
